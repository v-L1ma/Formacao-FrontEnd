...
export class TarefasView extends View<Tarefas> {

    protected template(model: Tarefas): string {
        ...
        return `
            <div id="gallery">
                ${model.listar().map(tarefa=>
                    `<div data-card="${tarefa.getId}" class="${tarefa.getFinalizada ? 'cardPerguntaFinalizada' : 'cardPergunta'}">
